...
console.log("Admin name found in sessionStorage:", adminName);
   ...
   debugger;
      ...
      displayadminname.innerHTML = `<span style="color: palevioletred; font-weight: bold;">welcome 👋: </span>${adminName}`;
...
function fetchProduct() {
  ...
  fetch(`http://localhost:5035/api/Products/${productId}`)
    ...
    document.getElementById("productImage").src = `http://localhost:5035/${product.imagePath}`;
      ...
      document.getElementById("productOccasion").textContent = product.occasion || "Not set";
...
function getAllCategories() {
  fetch('http://localhost:5035/api/Categories/GetAllCategories')
    .then(res => res.json())
    .then(data => {
...
function addCategory(event) {
  ...
  fetch('http://localhost:5035/api/Categories/AddCategory', {
    method: 'POST',
...
function deleteCategory() {
  ...
  fetch(`http://localhost:5035/api/Categories/DeleteCategory/${id}`, {
    method: 'DELETE'
  })
...
function updateCategory() {
  ...
  fetch(`http://localhost:5035/api/Categories/UpdateCategory/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
function getAllOccasions() {
  fetch('http://localhost:5035/api/Occasions/GetAllOccasions')
    ...
    .then(data => {
      ...
      list.innerHTML = '';
      data.forEach(occ => {
        list.innerHTML += `<p>ID: ${occ.id} - Name: ${occ.name}</p>`;
      });
    })
    .catch(err => console.error('Error:', err));
}

function addOccasion(event) {
  event.preventDefault();
  const name = document.getElementById('occasionNameInput').value;
  fetch('http://localhost:5035/api/Occasions/AddOccasion', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name })
  })
    .then(res => res.json())
    .then(data => alert('Occasion Added'))
    .catch(err => console.error('Error:', err));
}

function deleteOccasion() {
  const id = document.getElementById('occasionIdToDelete').value;
  fetch(`http://localhost:5035/api/Occasions/DeleteOccasion/${id}`, {
    method: 'DELETE'
  })
    .then(() => alert('Occasion Deleted'))
    .catch(err => console.error('Error:', err));
}

function updateOccasion() {
  const id = document.getElementById('occasionIdToUpdate').value;
  const newName = document.getElementById('newOccasionName').value;
  fetch(`http://localhost:5035/api/Occasions/UpdateOccasion/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ Id: id, Name: newName })
  })
    .then(() => alert('Occasion Updated'))
    .catch(err => console.error('Error:', err));
}

function populateOccasionsDropdown() {
  fetch('http://localhost:5035/api/Occasions/GetAllOccasions')
    .then(res => res.json())
    .then(data => {
      ...
      if (occasionSelect) {
        ...
        data.forEach(occ => {
          occasionSelect.innerHTML += `<option value="${occ.name}">${occ.name}</option>`;
        });
      }
    })
    .catch(err => console.error('Error loading occasions:', err));
}
// Load occasions for the dropdown automatically
